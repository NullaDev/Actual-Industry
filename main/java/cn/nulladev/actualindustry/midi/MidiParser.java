package cn.nulladev.actualindustry.midi;
...
public class MidiParser {
	...
	*/
	public MidiParser(File midiFile) throws MidiUnavailableException, IOException, InvalidMidiDataException {
	...
	*/
	public void writeMidiTextFile() {
		try {
			...
			int noteCount = 0;
			for (Note note : notes) {
				ps.println(note.toString() + "\n");
				noteCount++;
			}
		ps.close();
		this.successMessage = "<html><center>" + noteCount + " notes at a tempo of " + tempo + " " + "were exported at:<br>" + filePathAndName + "</center></html>";
